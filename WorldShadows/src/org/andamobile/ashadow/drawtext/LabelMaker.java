...
public class LabelMaker {
    ...
    public void beginDrawing(GL10 gl, float viewWidth, float viewHeight) {
    	...
    	if ( gl instanceof MatrixTrackingGL ) {
	    	mProjector.setCurrentView(0, 0, (int)viewWidth, (int)viewHeight);
     ...
     * @param labelID
     */
    public void draw(GL10 gl, float x, float y, int z, int labelID) {
    ...
    //Dibuja un label sobre el vértice de la vista OpenGL indicada
    //Para hacer la tralación de coordenadas OpenGL a las de pantalla. Para
    //eso hace falta la matriz de proyección de OpenGL y utilizarla para 
    //proyectar el vértice. Así se sabe donde pintar.
    public void draw(GL10 gl, float xVertex, float yVertex, float zVertex, int labelID, int z) {
    	if ( !(gl instanceof MatrixTrackingGL) )
    		draw( gl, 0, 0, labelID );
    	else {
	        mScratch[0] = xVertex;
	        ...
	        float sx = mScratch[4];
	        float sy = mScratch[5];
	        float height = getHeight(labelID);
	        float width = getWidth(labelID);
	        float tx = sx - width * 0.5f;
	        float ty = sy - height * 0.5f;
	        draw(gl, tx, ty, z, labelID);
    	}
    }
